...
start_backend() {
    ...
    local actual_port="${PORT:-$BACKEND_PORT}"
    local port_pid
    port_pid=$(lsof -t -i :"$actual_port" 2>/dev/null || true)
    if [ -n "$port_pid" ]; then
        print_warning "Port $actual_port is occupied by PID $port_pid. Freeing port..."
...
stop_backend() {
    ...
    # Also free the port to clean up any orphaned compilation processes
    local actual_port="${PORT:-$BACKEND_PORT}"
    local port_pid
    port_pid=$(lsof -t -i :"$actual_port" 2>/dev/null || true)
    if [ -n "$port_pid" ]; then
        print_info "Killing process $port_pid occupying port $actual_port..."
        kill -9 $port_pid 2>/dev/null || true
    fi
